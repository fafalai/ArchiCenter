...
function reorderImages()
{
    ...
    for (var i = 0; i < totalContainers.length; i++) {
        ...
        var element = [imgID, labelID, textID, rmBtnID, addBtnID, formID,'DilapidationImagesTable',rotateBtnID,angleID,'DilapidationPhotographs'];
        ...
        var removeFunction  = "DeleteOneImg(['" + imgID + "','" + labelID+"','" + textID + "','" + rmBtnID +"','" +  addBtnID + "','" + formID + "','" + 'DilapidationImagesTable' + "','"+ rotateBtnID +  "','" + angleID +  "','"+'DilapidationPhotographs' + "'])";
        var rotateFunction = "RotateOneImage(['" + imgID + "','" + labelID+"','" + textID + "','" + rmBtnID +"','" +  addBtnID + "','" + formID + "','" + 'DilapidationImagesTable' + "','"+ rotateBtnID +  "','" + angleID + "','"+'DilapidationPhotographs' + "'])";

        //console.log(removeFunction);
        $("#" + addBtnID).click(function () {
            global_Img = element;
            //console.log(global_Img);
            $("#DilapidationUploadOneImage").click();
        });
...
function addNewImageForm()
{
    ...
    if (totalContainers.length < maxImage && totalContainers.length != 0) {
        ...
        var nextLabelID = 'CDilapidationImageLable' + newID;
        var nextLableValue = 'IMG' + (parseInt(newID) + 1);
        var nextRemoveButtonID = 'CDilapidationImageRemoveButton' + newID;
        var nextAddButtonID = 'AddDilapidationImageButton' + newID;
        var nextUploadID = 'DilapidationUploadImage' + newID;
        var nextRotateBtnID = 'DilapidationImageRotateButton' + newID;
        var nextAngelInputID = 'DilapidationImageAngle' + newID;
        var nextFormID = "DilapidationImageForm" + newID;
       
        var emptyElement = createImagesElements('DilapidationImagesTable', nextImageID, nextLabelID, nextLableValue, nextTextID, nextRemoveButtonID, nextAddButtonID, nextFormID,nextRotateBtnID,nextAngelInputID,'DilapidationPhotographs');
...
$('#DilapidationUploadCoverImage').change(function(){
    ...
    if (this.files && this.files[0]) {
        ...
        loadImage.parseMetaData(imageFile, function (data) {
            ...
            var rotateButton = document.getElementById('DilapidationCoverImageRotateButton');
            document.getElementById("DilapidationCoverImageAngle").value = "";
            $("#DilapidationCoverImage").rotate(0);
            ...
            var loadingImage = loadImage(imageFile, function (canvas) {
                ...
                doUploadFile(file, 'DilapidationCoverImage', '', 'DilapidationCoverImageRemoveButton', '', '', 'cover image', '', '', '', '', '265px', '400px','DilapidationCoverImageRotateButton',"DilapidationCoverImageAngle");
...
function RotateDilapidationCoverImage()
{
    var rotateAngle;
    var originalAngle = document.getElementById('DilapidationCoverImageAngle').value;
    ...
    var myImage = document.getElementById('DilapidationCoverImage');
    ...
    if(rotateAngle == 90 || rotateAngle == 270)
    {
        ...
        $("#DilapidationCoverImage").rotate(rotateAngle);
    }
    else
    {
    ...
    document.getElementById('DilapidationCoverImageAngle').value = rotateAngle
}
...
function RotateSavedCoverImage()
{
    ...
    var myImage = document.getElementById('DilapidationCoverImage');
    var originalAngle = parseInt(document.getElementById('DilapidationCoverImageAngle').value);
    var rotateBtn = document.getElementById('DilapidationCoverImageRotateButton');
    ...
    if (myImage.src.includes("photos/") > 0) 
    {
        ...
        if(originalAngle == 90 || originalAngle == 270)
        {
            ...
            myImage.style.marginTop = "100px";
            myImage.style.marginBottom = "100px";
            $("#DilapidationCoverImage").rotate(originalAngle);
        }
...
$('#DilapidationUploadImages').change(function () {
    ...
    if (this.files.length >=60) 
    {
        ...
        Object.keys(allImages).forEach(i => 
        {
            ...
            var imageID = 'DilapidationImage' + elementID;
            var labelID = 'DilapidationImageLable' + elementID;
            var labelValue = 'IMG' + elementID;
            var textID = 'DilapidationImageText' + elementID;
            var removeButtonID = 'DilapidationImageRemoveButton' + elementID;
            var addButtonID = 'AddDilapidationImageButton' + elementID;
            var uploadID = 'DilapidationUploadImage' + elementID;
            //var imgLabelID = "imageCaption" + newid;
            var rotateButtonID = 'ADilapidationImageRotateButton' + elementID;
            var imgAngleInputID = "DilapidationImageAngle" + elementID;
            var formID = "DilapidationImageForm" + elementID;
            //Create elements
            //[containerID,imgID, labelID, labelValue, textID, rmBtnID, addBtnID, formID]
            var element = createImagesElements("DilapidationImagesTable", imageID, labelID, labelValue,
            textID, removeButtonID, addButtonID, formID,rotateButtonID,imgAngleInputID,'DilapidationPhotographs');
...
$('#DilapidationUploadOneImage').click(function () {
   
    this.value = null;
});
$("#DilapidationUploadOneImage").on('change', function (e) {
    console.log("DilapidationUploadOneImage onchange");
...
function createEmptElementForAddingImg(newID)
{
    var nextImageID = 'DilapidationImage' + newID;
    var nextTextID = 'DilapidationImageText' + newID;
    var nextLabelID = 'DilapidationImageLable' + newID;
    var nextLableValue = 'IMG' + (parseInt(newID) + 1);
